...
public class AuthService {
    ...
    public AuthenticationResponse register(RegistrationRequest request) {
        // Check if payment has been made using paymentId
        if (request.getPaymentId() == null || request.getPaymentId().isEmpty()) {
            throw new RuntimeException("Registration fee payment required. Please complete payment first.");
        }

        if (!registrationPaymentService.isPaymentValidForRegistration(request.getPaymentId())) {
            throw new RuntimeException("Invalid or expired payment. Please complete payment again.");
        }
